...
RNode AngCoeff::doSomething(RNode d){
    ...
    // only once 
    auto hDenominator = d7.Histo2D(TH2D("hdenom", "hdenom", nBinsY, yArr, nBinsPt, ptArr), "Wpt_preFSR", "Wrap_preFSR", "Generator_weight_norm");
    
    for(auto c:coeff){

	auto hNumerator = d7.Histo2D(TH2D(Form("A%s", c.c_str()), Form("A%s", c.c_str()), nBinsY, yArr, nBinsPt, ptArr), "Wpt_preFSR", "Wrap_preFSR", "P"+c+"w");
	auto hP2 =  d7.Histo2D(TH2D(Form("hnum2_%s", c.c_str()), Form("hnum2_%s", c.c_str()), nBinsY, yArr, nBinsPt, ptArr), "Wpt_preFSR", "Wrap_preFSR", "P"+c+"w2");
